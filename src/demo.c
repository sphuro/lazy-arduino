...
const char *sketches[MAX_SKETCHES] = {"blink.ino", "servo.ino", "adc.ino",
                                      "wifi.ino", "temp.ino"};
...
void draw_content() {
  ...
  for (int i = 0; i < MAX_SKETCHES; i++) {
    if (i == selected_sketch)
      wattron(sketches_win, A_REVERSE);
    mvwprintw(sketches_win, i + 1, 2, "%s", sketches[i]);
    if (i == selected_sketch)
      wattroff(sketches_win, A_REVERSE);
  }
  ...
  int board_count = get_boards(boards, MAX_BOARDS);

  if(board_count <=0){
    mvwprintw(boards_win, 1,2, "No boards found.");
  }else{
    for(int i = 0; i< board_count; i++){
      mvwprintw(boards_win, i+1, 2, "%s (%s)", boards[i].boardName, boards[i].port);
    }
  }
  ...
  mvwprintw(status_win, 0, 1,
            "[Tab] Focus  [e] Edit  [c] Compile  [u] Upload  [q] Quit");
...
int main() {
  ...
  while ((ch = getch()) != 'q') {
    ...
    case KEY_DOWN:
    case 'j':
      if (selected_sketch < MAX_SKETCHES - 1)
        selected_sketch++;
      break;
    case 'e':
